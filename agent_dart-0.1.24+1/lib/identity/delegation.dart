...
class Delegation extends ToCBorable {
  ...
  CborValue toCbor() {
    final map = <CborValue, CborValue>{
      CborString('pubkey'): CborBytes(Uint8List.fromList(pubkey)),
      CborString('expiration'): CborBigInt(expiration),
    };
    if (targets != null) {
      map[CborString('targets')] = CborList(
        targets!
            .map((e) => CborBytes(Uint8List.fromList(e.toUint8Array())))
            .toList(),
      );
    }
    return CborMap(map);
