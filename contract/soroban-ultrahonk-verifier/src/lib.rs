...
mod utils;
use utils::{UltraHonkVerifier, VkLoadError, PROOF_BYTES};

use crate::utils::verifier::VerifyError;
...
impl UltraHonkVerifierContract {
    ...
    pub fn verify_proof(env: Env, public_inputs: Bytes, proof_bytes: Bytes) -> Result<(), Error> {
        ...
        verifier
            ...
            .map_err(|e| match e {
                VerifyError::InvalidInput => Error::InvalidPublicInputs,
                VerifyError::SumcheckFailed => Error::SumcheckFailed,
                VerifyError::ShplonkFailed => Error::ShplonkFailed,
            })?;
        Ok(())
    }
}
