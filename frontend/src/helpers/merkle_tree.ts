...
async function hashLeftRight(left: string, right: string): Promise<string> {
  const hash = poseidon2Hash([BigInt(left), BigInt(right)]);
  return hash.toString();
}

type HashFn = (left: string, right: string) => Promise<string>;
...
export class PoseidonTree {
  ...
  proof(index: number): ProofResult {
  ...
  async update(
    ...
    await this.traverseAsync(
      index,
      async (level: number, currentIndex: number, siblingIndex: number) => {
        const sibling =
          this.storage.get(PoseidonTree.indexToKey(level, siblingIndex)) ||
          this.zeros[level];
        const [left, right] =
