...
class StacksCoin extends Coin {
  ...
  Future<AccountData> fromMnemonic({required String mnemonic}) async {
    final cacheKey =
        'stackscDetail$default_${walletImportType.name}$derivationPath';
  ...
  Future<AccountData> fromPrivateKey(String privateKey) async {
    ...
    final pubBytes = compressedPubKey(privBytes);
    final address =
  ...
  Future<Uint8List> signMessage(String message, {bool isLegacy = false}) async {
...
Map<String, dynamic> calculateStacksKey(StacksDeriveArgs args) {
  ...
  final pubKeyBytes = compressedPubKey(node.privateKey!);
