...
Map calculateBitCoinKey(BitcoinDeriveArgs config) {
  ...
  if (config.default_ == 'STX') {
    // Decode the base58check address to get the raw 20-byte hash160
    final decoded = bs58check.decode(address);
    // decoded[0] is the version byte — skip it, take the 20-byte pubKeyHash
    final pubKeyHashHex = HEX.encode(decoded.sublist(1));

    // Version 22 (0x16) = mainnet P2PKH → SP... address
    address = c32checkEncode(22, pubKeyHashHex);
  }
