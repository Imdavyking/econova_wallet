...
class SSS {
  ...
  List<List<List<BigInt>>> decodeShareBase64(List<String> shares) {
    ...
    for (int i = 0; i < shares.length; i++) {
      ...
      if (isValidShareBase64(shares[i]) == false) {
        throw Exception("one of the shares is invalid");
      }
  ...
  /// Returns a array of secret shares (encoding x,y pairs as Base64 or Hex strings)
  ...
  List<String> create(int minimum, int shares, String secret, bool isBase64) {
    ...
    if (minimum <= 0 || shares <= 0) {
      throw Exception("minimum or shares is invalid");
    }
    if (minimum > shares) {
      throw Exception("cannot require more shares then existing");
    }
    if (secret.isEmpty) {
      throw Exception("secret is NULL or empty");
    }
    ...
    // BigInt[][] polynomial = BigInt[secrets.size()][minimum];
  ...
  String combine(List<String> shares, bool isBase64) {
    String rs = "";
    if (shares.isEmpty) {
      throw Exception("shares is NULL or empty");
    }
