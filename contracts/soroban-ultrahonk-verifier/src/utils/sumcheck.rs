...
pub fn verify_sumcheck(
    ...
    if grand_honk_relation_sum == round_target {
        ...
        crate::trace!(
            "grand_relation = 0x{}",
            crate::utils::debug::Hex(&grand_honk_relation_sum.to_bytes())
        );
        crate::trace!("target = 0x{}", crate::utils::debug::Hex(&round_target.to_bytes()));
        crate::trace!(
            "difference = 0x{}",
            crate::utils::debug::Hex(&(grand_honk_relation_sum - round_target).to_bytes())
        );
        crate::trace!("======================================");
